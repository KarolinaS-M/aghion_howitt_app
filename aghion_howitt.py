...
def growth_without_scale_effects(alpha, gamma, lambd, sigma, psi, eps):
    """g in variable-variety model (profit term proportional to ε/ψ)."""
    pi_val = pi_drastic(alpha)
...
def main():
    ...
    alpha = st.sidebar.slider("α (output elasticity)", 0.01, 0.99, 0.30, 0.01)
    gamma = st.sidebar.slider("γ (quality step)", 1.01, 1.50, 1.20, 0.01)
    lambd = st.sidebar.slider("λ (research productivity)", 0.10, 1.00, 0.50, 0.05)
    sigma = st.sidebar.slider("σ (elasticity in φ)", 0.01, 0.99, 0.50, 0.01)
    ...
    if mode == "One-sector: drastic innovations":
        ...
        with col1:
            ...
            st.write(f"π = {pi_val:.4f}")
            st.write(f"μ (theoretical) = {mu_theory:.4f}")
    ...
    # -------------------------------------------------------------
        ...
        chi = st.sidebar.slider("χ (limit price)", 1.05, 4.0, CHI_DEFAULT, 0.05)
        ...
        axes[0].set_title("Profit parameter π(χ)")
    ...
    # -------------------------------------------------------------
        ...
        psi = st.sidebar.slider("ψ (discovery prob per person)", 0.01, 0.99, PSI_DEFAULT, 0.01)
        eps = st.sidebar.slider("ε (disappearance fraction)", 0.01, 0.99, EPS_DEFAULT, 0.01)
