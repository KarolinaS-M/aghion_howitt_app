...
def simulate_multisector(A0, T, alpha, gamma, lambd, sigma, L,
    ...
    y_factor = alpha ** (2.0 * alpha / (1.0 - alpha))
    x_factor = alpha ** (2.0 / (1.0 - alpha))
    gdp_factor = y_factor * (1.0 - alpha ** 2)
...
def simulate_M_path(L_value, psi, eps, T, M0):
    """Law of motion for number of varieties: M_{t+1} - M_t = psi * L - eps * M_t."""
    N = T
    M = np.empty(N + 1)
    M[0] = M0
    for t in range(N):
...
def main():
    ...
    if mode == "One-sector: drastic innovations":
        ...
        chi = st.sidebar.slider("chi (limit price)", 1.05, 4.0, chi_baseline, 0.05)
        sim_nd = simulate_nondrastic_one_sector(A0, T, alpha, gamma, lambd, sigma, L, chi, seed=int(seed))
        sim_d = simulate_drastic_one_sector(A0, T, alpha, gamma, lambd, sigma, L, seed=int(seed))
        ...
        sim_d = simulate_drastic_one_sector(A0, T, alpha, gamma, lambd, sigma, L, seed=int(seed))
        sim_m = simulate_multisector(A0, T, alpha, gamma, lambd, sigma, L,
        ...
        # grid for L and theoretical g(L) in one-sector model
        ...
        # dynamics of M_t for a chosen L
