...
def main():
    ...
    λ = st.sidebar.slider("λ (research productivity)", 0.10, 1.00, 0.50, 0.10)
    ...
    if mode == "One-sector: drastic innovations":
        ...
        # NOW: nicer grids
        lambda_grid = np.arange(0.1, 1.0 + 1e-9, 0.1)
        gamma_grid = np.arange(1.1, 1.5 + 1e-9, 0.05)
        alpha_grid = np.arange(0.1, 0.9 + 1e-9, 0.1)
